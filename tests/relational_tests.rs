...
pub async fn linked() -> Result<(), DbErr> {
    ...
    let baked_for_customers: Vec<(BakerLite, Option<CustomerLite>)> = Baker::find()
        ...
        .column_as(
            Expr::col((Alias::new("r4"), customer::Column::Name)).into(),
            (SelectB, customer::Column::Name),
        )
        .group_by(baker::Column::Id)
        .group_by(Expr::col((Alias::new("r4"), customer::Column::Id)).into())
        .group_by(baker::Column::Name)
        .group_by(Expr::col((Alias::new("r4"), customer::Column::Name)).into())
        .order_by_asc(baker::Column::Id)
        .order_by_asc(Expr::col((Alias::new("r4"), customer::Column::Id)).into())
        .into_model()
        .all(&ctx.db)
        .await?;
