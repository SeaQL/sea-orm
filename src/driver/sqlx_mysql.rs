...
use crate::{
    ...
    DbBackend, IsolationLevel, QueryStream, Statement, TransactionConfig, TransactionError,
    debug_print, error::*, executor::*,
};
...
impl SqlxMySqlPoolConnection {
    ...
    pub async fn begin(&self, config: TransactionConfig) -> Result<DatabaseTransaction, DbErr> {
        let conn = self.pool.acquire().await.map_err(sqlx_conn_acquire_err)?;
        DatabaseTransaction::new_mysql(conn, self.metric_callback.clone(), config).await
    }
        ...
        E: std::fmt::Display + std::fmt::Debug + Send,
    {
        let conn = self.pool.acquire().await.map_err(sqlx_conn_acquire_err)?;
        let transaction =
            DatabaseTransaction::new_mysql(conn, self.metric_callback.clone(), config)
                .await
