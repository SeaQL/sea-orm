    ...
    fn value_type_try_from_impl(&self) -> TokenStream {
        if self.generate_enum_impls() {
            quote! {
                ...
                match v {
                    Value::Enum(value) => match value.as_ref() {
                        OptionEnum::Some(value) => <Self as sea_orm::ActiveEnum>::try_from_value(value)
    ...
    fn nullable_impl(&self) -> TokenStream {
        ...
        let nullable_value_impl = if self.generate_enum_impls() {
            quote! {
                ...
                Value::Enum(Box::new(OptionEnum::None(#enum_name.into())))
            }
        } else {
            quote! {
