...
impl RelationVariants {
    ...
    fn forward_ident(&self, entity: &TypePath) -> Ident {
        if let Some(name) = self.explicit_name() {
            format_ident!("{}", name.value().to_upper_camel_case())
        } else {
            format_ident!(
                "{}",
    ...
    fn reverse_ident(&self, forward: &Ident) -> Option<Ident> {
        ...
        } = self
        {
            Some(if let Some(reverse_variant) = reverse_variant {
                format_ident!("{}", reverse_variant.value())
            } else {
                format_ident!("{}Reverse", forward)
            })
        } else {
            None
        }
...
fn expand_belongs_to_into(
    ...
    if let Some(relation) = &attr.relation {
        ...
        let related_entity_lit = entity
            ...
            .collect::<Vec<_>>()
            .join("::");
        let belongs_to = format_ident!("belongs_to");
...
fn expand_has_one_into(
    ...
    if attr.has_one {
        ...
        let related_entity_lit = entity
            ...
            .collect::<Vec<_>>()
            .join("::");
        let has_one = format_ident!("has_one");
        let mut extra: Punctuated<_, Comma> = Punctuated::new();
        if let Some(via_rel) = &attr.via_rel {
            let tag = format_ident!("via_rel");
...
fn expand_has_many_into(
    ...
    {
        let has_many = format_ident!("has_many");
...
fn expand_impl_related_many_to_many_self(
    ...
    match direction {
        ManyToManySelfDirection::Forward { from, to } => {
            let from = format_ident!("{}", from.value());
            let to = format_ident!("{}", to.value());
        ...
        if columns.len() > 1 {
            ...
            let filters = columns
                ...
                .map(|(i, (col, _))| {
                    ...
                    let col = format_ident!("{}", col.to_string().to_upper_camel_case());
