...
pub fn expand_derive_active_model(ident: Ident, data: Data) -> syn::Result<TokenStream> {
    ...
    Ok(quote!(
        ...
        impl ActiveModel {
            pub async fn save(self, db: &sea_orm::DatabaseConnection) -> Result<Self, sea_orm::OrmError> {
                sea_orm::save_active_model::<Self, Entity>(self, db).await
            }

            pub async fn delete(self, db: &sea_orm::DatabaseConnection) -> Result<sea_orm::DeleteResult, sea_orm::OrmError> {
