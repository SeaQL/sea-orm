...
pub fn expand_derive_active_model_ex(
    ...
    let active_model_action =
        expand_active_model_action(&belongs_to_fields, &has_one_fields, &has_many_fields);
...
fn expand_active_model_action(
        ...
        has_many_action.extend(quote! {
            if #field.is_replace() {
                ...
                for redundant in redundant {
                    let mut found = false;
                    let pk = redundant.get_primary_key_value();
                    for current in #field.as_slice() {
                        if let Some(pkc) = current.get_primary_key_value() {
                            if pkc == pk {
                                found = true;
                                break;
                            }
    ...
    quote! {
        ...
        #[doc = " Generated by sea-orm-macros"]
        pub async fn action<'a, C>(mut self, action: sea_orm::ActiveModelAction, conn: &'a C) -> Result<Self, DbErr>
        where
            C: sea_orm::TransactionTrait,
        {
            use sea_orm::{HasOneModel, HasManyModel, IntoActiveModel, ConnectionTrait, TransactionSession};
