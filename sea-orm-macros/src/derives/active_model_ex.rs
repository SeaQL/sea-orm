...
fn expand_active_model_action(
    ...
    for field in has_one {
        ...
        has_one_action.extend(quote! {
            match #field {
                HasOneModel::NotSet => {}
                // `Set` (replace) or `Delete`: remove the existing linked record unless it is
                // the same one being set. `NotSet` leaves it untouched (non-destructive default).
                related => {
                    if let Some(item) = model.find_related_of(related.as_slice()).one(db)#await_? {
                        if !related.find(&item) {
                            #delete_associated_model
                        }
                    }
                    if let Some(mut child) = related.into_option() {
                        child.set_parent_key(&model)?;
                        if child.is_changed() {
                            model.#field = HasOneModel::set(#box_pin(child.action(action, db))#await_?);
                        } else {
                            model.#field = HasOneModel::set(child);
                        }
                    } else {
                        model.#field = HasOneModel::NotSet;
                    }
                }
            }
        });
                        ...
                        if field_type_str.starts_with("HasOne<") {
                            ...
                            setters.extend(quote! {
                                ...
                                pub fn #setter_option(mut self, v: Option<impl Into<#active_model_type>>) -> Self {
                                    self.#ident = match v {
                                        Some(v) => sea_orm::HasOneModel::set(v.into()),
                                        None => sea_orm::HasOneModel::Delete,
                                    };
                                    self
                                }
                                ...
                                pub fn #deleter(mut self) -> Self {
                                    self.#ident = sea_orm::HasOneModel::Delete;
                                    self
                                }
                            });
                        } else {
