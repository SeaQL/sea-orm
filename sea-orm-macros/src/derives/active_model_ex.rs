...
pub fn expand_derive_active_model_ex(
    ...
    Ok(quote! {
        ...
        impl Model {
            #[doc = " Generated by sea-orm-macros"]
            pub async fn cascade_delete<'a, C>(self, db: &'a C) -> Result<sea_orm::DeleteResult, sea_orm::DbErr>
            where
        ...
        impl ModelEx {
            #[doc = " Generated by sea-orm-macros"]
            pub async fn delete<'a, C>(self, db: &'a C) -> Result<sea_orm::DeleteResult, sea_orm::DbErr>
            where
                C: sea_orm::TransactionTrait,
            {
...
fn expand_active_model_action(
    ...
    let delete_associated_model = quote! {
        ...
        if item.clear_parent_key::<Entity>()? {
            ...
            deleted.merge(item.into_ex().delete(db).await?); // deep delete
        }
    };
    ...
    quote! {
        ...
        #[doc = " Generated by sea-orm-macros"]
        pub async fn action<'a, C>(mut self, action: sea_orm::ActiveModelAction, db: &'a C) -> Result<Self, sea_orm::DbErr>
        where
            C: sea_orm::TransactionTrait,
        {
