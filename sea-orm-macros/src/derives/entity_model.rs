            ...
            for field in fields.named {
                if let Some(ident) = &field.ident {
                    ...
                    if is_compound_field(&field_type) {
                        let attrs = compound_attr::SeaOrm::from_attributes(&field.attrs)?;
...
fn impl_related_trait(attr: compound_attr::SeaOrm, ty: String) -> TokenStream {
    if let Some(related) = attr.related {
        let related = related.value();
        let related_entity: TokenStream = if ty.starts_with("BelongsTo<") {
            ...
            panic!("`related` attribute applied to non compound type: {ty}")
        }
        .parse()
        .unwrap();
