...
impl EntityWriter {
    pub fn generate(self, context: &EntityWriterContext) -> WriterOutput {
        ...
        files.push(self.write_index_file(context));
        if context.with_prelude != WithPrelude::None {
    ...
    pub fn write_index_file(&self, context: &EntityWriterContext) -> OutputFile {
        let mut lines = Vec::new();
        Self::write_doc_comment(&mut lines);
        ...
        let file_name = match context.lib {
