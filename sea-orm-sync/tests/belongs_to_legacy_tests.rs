...
#[sea_orm_macros::test]
fn test_legacy_belongs_to_nested_write() -> Result<(), DbErr> {
    let ctx = setup("test_legacy_belongs_to_nested_write");
    let db = &ctx.db;
    ...
    let profile_row = profile::Entity::find().one(db)?.expect("profile");
    ...
    assert_eq!(user::Entity::find().count(db)?, 2);
...
#[sea_orm_macros::test]
fn test_legacy_many_to_many() -> Result<(), DbErr> {
    let ctx = setup("test_legacy_many_to_many");
    let db = &ctx.db;
...
#[sea_orm_macros::test]
fn test_legacy_belongs_to_duplicate_target() -> Result<(), DbErr> {
    let ctx = setup("test_legacy_belongs_to_duplicate_target");
    let db = &ctx.db;
...
#[sea_orm_macros::test]
fn test_legacy_entity_loader() -> Result<(), DbErr> {
    let ctx = setup("test_legacy_entity_loader");
    let db = &ctx.db;
