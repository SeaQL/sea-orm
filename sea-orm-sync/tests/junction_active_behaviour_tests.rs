...
mod jpost_tag {
    ...
    pub struct Model {
        ...
        pub post: BelongsTo<super::jpost::Entity>,
        #[sea_orm(belongs_to, from = "tag_id", to = "id")]
        pub tag: BelongsTo<super::jtag::Entity>,
    }
    ...
    impl ActiveModelBehavior for ActiveModel {
        fn before_save<C: ConnectionTrait>(self, db: &C, _insert: bool) -> Result<Self, DbErr> {
            let m = self.clone().try_into_model()?;
            log(db, "before_save", m.post_id, m.tag_id)?;
            Ok(self)
        }
        ...
        fn after_save<C: ConnectionTrait>(
            ...
            log(db, "after_save", model.post_id, model.tag_id)?;
            Ok(model)
        }
    }
}
...
#[sea_orm_macros::test]
fn junction_active_behaviour() -> Result<(), DbErr> {
    let ctx = TestContext::new("junction_active_behaviour_tests");
    let db = &ctx.db;
    ...
    let logs = jlog::Entity::find().all(db)?;
