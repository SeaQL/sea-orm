    ...
    ActiveModel: ActiveModelTrait,
{
    for col in columns.iter() {
        ...
        match model.get(column).into_value() {
            ...
            // The key column was never set (e.g. a freshly-built ActiveModel):
            // there is nothing to clear — it will insert as NULL / stay absent on
            // update — so treat it as already cleared rather than erroring.
            None => {}
        }
    }
